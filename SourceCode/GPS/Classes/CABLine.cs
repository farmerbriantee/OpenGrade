...
namespace OpenGrade
{
    public class CABLine
    {
        ...
        public void GetCurrentABLine()
        {
            ...
            pivotAxlePosAB.easting = mf.pn.easting;
            pivotAxlePosAB.northing = mf.pn.northing;

            //move the ABLine over based on the overlap amount set in vehicle
            double widthMinusOverlap = mf.vehicle.toolWidth;
            ...
            {
                point1 = new vec2((Math.Cos(-abHeading) * ((widthMinusOverlap * howManyPathsAway * refLineSide))) + refPoint1.easting,
                (Math.Sin(-abHeading) * ((widthMinusOverlap * howManyPathsAway * refLineSide))) + refPoint1.northing);
            }
            else
            {
                point1 = new vec2((Math.Cos(-abHeading) * ((widthMinusOverlap * howManyPathsAway * refLineSide))) + refPoint1.easting,
